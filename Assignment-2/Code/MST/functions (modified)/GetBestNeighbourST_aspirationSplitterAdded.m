function [BestNeighbourST, BestNeighbourSTCost, TabuEdges]... 
          = GetBestNeighbourST_splitterAdded(Graph, ST, TabuEdges, TabuLength)
    ...
    for e1 = 1:NumTreeEdges
        ...
        % --- compute the additional cost for splitters --- %
        NewSTCost = NewSTCost + splitterCost(NewST);
        % --- compute the additional cost for splitters --- %
        
        
        
        % 2. Check if the edge to be removed (e1) is in the tabu list
        if TabuEdges(NT1(e1), NT2(e1)) == 0 || isAspirating(NewSTCost, BestNeighbourSTCost, TabuEdges(NT1(e1), NT2(e1)), TabuLength)
